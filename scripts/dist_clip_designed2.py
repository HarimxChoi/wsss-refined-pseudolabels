...
def train(cfg):
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        use_teacher_every = 1 if (n_iter < post_cam_warmup) else int(getattr(cfg, "ema_uncert_every", 2))
        ...
        if use_teacher_now:
            with torch.no_grad():
                if teacher_down < 1.0:
                    ...
                    t_in = inputs
                # fused logits from EMA teacher
                t_logits_for_uncert = ema_teacher.forward_logits(t_in, img_name)
                teacher_prob_for_cam = F.softmax(t_logits_for_uncert, dim=1)
        # model teacher prob injection
        ...
        with torch.no_grad():
            if use_teacher_now and (t_logits_for_uncert is not None):
                ...
                if t_logits_up.shape[2:] != pseudo_label.shape[1:]:
                    t_logits_up = F.interpolate(t_logits_up, size=pseudo_label.shape[1:], mode='bilinear', align_corners=False)
        ...
        if (n_iter + 1) % cfg.train.log_iters == 0:
            ...
            teacher_stats = {
                ...
                'high_conf_active': (n_iter >= model.high_conf_merge_iter)
            }
            ...
            if teacher_prob_for_cam is not None:
                with torch.no_grad():
                    ...
                    teacher_stats['high_conf_ratio'] = float((t_conf >= model.teacher_cam_tau).float().mean().item())
            ...
            with torch.no_grad():
                ...
                urn_stats = {
                    ...
                    'w_urn_min': float(w_urn.min().item()),
                    'w_urn_mean': float(w_urn.mean().item()),
            ...
            logging.info(
                ...
                "L_ce:%.4f L_bnd:%.4f L_kl:%.4f attn:%.4f mAcc:%.4f | "
                "gamma:%.3f edge:%.3f | "
                "T[%s/%d conf:%.3f]"
                % (
                    n_iter + 1, delta, eta, cur_lr,
                    float(L_ce.detach().item()),
            ...
            writer.add_scalar('train/gamma_t', float(gamma_t), n_iter)
            writer.add_scalar('train/edge_ratio', edge_ratio, n_iter)

            
            
            # === URN ===
            writer.add_scalars('train/urn', {
                "w_urn_min": urn_stats['w_urn_min'],
                "w_urn_mean": urn_stats['w_urn_mean']
            }, n_iter)
            
            # === Teacher ===
            writer.add_scalar('train/teacher_active', float(teacher_stats['active']), n_iter)
            writer.add_scalar('train/teacher_high_conf_mode', float(teacher_stats['high_conf_active']), n_iter)
            if 'conf_mean' in teacher_stats:
                writer.add_scalars('train/teacher_conf', {
                    "mean": teacher_stats['conf_mean'],
                    "max": teacher_stats['conf_max'],
                    "high_ratio": teacher_stats['high_conf_ratio']
                }, n_iter)
        ...
        if (n_iter + 1) % 5000 == 0:
            with torch.no_grad():
                ...
                if teacher_prob_for_cam is not None:
                    ...
                    logging.info(f"  High-conf pixels (>={model.teacher_cam_tau}): {(t_conf >= model.teacher_cam_tau).float().mean():.2%}")
        ...
        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ...
            state = {
                ...
                "ema_tau": ema_teacher.tau if ema_teacher else None,
