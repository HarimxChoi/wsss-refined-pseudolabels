...
def train(cfg):
    ...
    beco_alpha = float(getattr(cfg, "beco_alpha", 0.3))
    ...
    use_teacher_every = int(getattr(cfg, "ema_uncert_every", 2))
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        segs_clip1, segs_dino1, cam, attn_pred = model(inputs.cuda(), img_name)
        
        pseudo_label = cam  # [B,H,W]

        segs1 = F.interpolate(
            0.5 * segs_clip1 + 0.5 * segs_dino1,
            size=pseudo_label.shape[1:], mode='bilinear', align_corners=False
        )
        
        # --- uncertainty u compute ---
        with torch.no_grad():
            use_teacher = (ema_teacher is not None) and (use_teacher_every >= 1) and ((n_iter + 1) % use_teacher_every == 0)
            if use_teacher:
                if teacher_down < 1.0:
                    b, c, h, w = inputs.shape
                    th, tw = max(1, int(h * teacher_down)), max(1, int(w * teacher_down))
                    t_in = F.interpolate(inputs.cuda(), size=(th, tw), mode='bilinear', align_corners=False)
                    t_logits = ema_teacher.forward_logits(t_in, img_name)
                    t_logits = F.interpolate(t_logits, size=pseudo_label.shape[1:], mode='bilinear', align_corners=False)
                else:
                    t_logits = ema_teacher.forward_logits(inputs.cuda(), img_name)
                    t_logits = F.interpolate(t_logits, size=pseudo_label.shape[1:], mode='bilinear', align_corners=False)
                u_map = _sym_kl_from_logits(t_logits, segs1, T=urn_T)  # teacher-student KL
            else:

                p = F.softmax(segs1 / max(urn_T, 1.0), dim=1)
                u_map = -(p * (p.clamp_min(1e-8)).log()).sum(1)        # [B,H,W]
            u_hat = _minmax01(u_map)                                   # [0,1]
        

        with torch.no_grad():

            w_cls = _class_weights_from_label(pseudo_label, cfg.dataset.num_classes).to(segs1.device)  # [C]
            w_wce = w_cls[pseudo_label.clamp_min(0)]                                                   # [B,H,W]
            w_wce[pseudo_label == cfg.dataset.ignore_index] = 0.0
        

            edge_map = _fast_edge_from_label(pseudo_label, ignore_idx=cfg.dataset.ignore_index)         # [B,H,W]
        ...
        ce1 = F.cross_entropy(segs1, pseudo_label.long(), ignore_index=cfg.dataset.ignore_index, reduction='none')
        L_ce_b2 = (ce1 * w_prime * valid).sum() / denom
        

        boundary_logits = _boundary_logits_from_seg_logits(segs1, tau=0.5)
        ...
        L_kl_soft = segs1.new_tensor(0.0)
        ...
        # --- final loss ---
        loss = L_ce_b2 + L_boundary + L_kl_soft + lambda_attn * attn_loss


        avg_meter.add({
            'L_ce_b2': float(L_ce_b2.detach().item()),
        ...
        if (n_iter + 1) % cfg.train.log_iters == 0:
            ...
            segs_mean = segs1
            preds = torch.argmax(segs_mean, dim=1).cpu().numpy().astype(np.int16)
            gts = pseudo_label.cpu().numpy().astype(np.int16)
            seg_mAcc = (preds == gts).sum() / preds.size


            logging.info(
                "Iter:%d Elapsed:%s ETA:%s LR:%.3e | L_ce_b2:%.4f L_bnd:%.4f L_kl:%.4f attn:%.4f gamma:%.2f"
                % (n_iter + 1, delta, eta, cur_lr,
                   float(L_ce_b2.detach().item()),
                   float(L_boundary.detach().item()),
                   float(L_kl_soft.detach().item()),
                   float(attn_loss.detach().item()),
                   gamma_t)
            )

            writer.add_scalar('train/mAcc', float(seg_mAcc), n_iter)
            writer.add_scalars('train/loss', {
                "L_ce_b2": float(L_ce_b2.detach().item()),
                "L_boundary": float(L_boundary.detach().item()),
                "L_kl": float(L_kl_soft.detach().item()),
                "attn": float(attn_loss.detach().item())
            }, n_iter)

        
        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ckpt_name = os.path.join(cfg.work_dir.ckpt_dir, f"wetr_iter_{n_iter+1}.pth")
            state = {"student": model.state_dict()}
            if ema_teacher is not None:
                state["ema_teacher"] = ema_teacher.teacher.state_dict()
                state["ema_tau"] = ema_teacher.tau
            torch.save(state, ckpt_name)
        
            logging.info('Validating (quick single-scale)...')
            seg_score, _ = validate_quick(model, val_dataset, cfg,
                                          resize_long=getattr(cfg.clip_init, "resize_long", None),
                                          max_images=getattr(cfg, "val_max_images", None),
                                          num_workers=2)
            logging.info(f"[VAL] mIoU: {seg_score['miou']:.4f}  pAcc: {seg_score['pAcc']:.4f}  mAcc: {seg_score['mAcc']:.4f}")
